-- Prawnik AI - Complete Database Schema
-- PostgreSQL 15+ with pgvector extension
-- Multi-tenant SaaS dla kancelarii prawnych
-- Migration: Initial Schema
-- Date: 2024-11-16
-- Note: Extensions are enabled in previous migration
...
CREATE INDEX idx_matters_deleted ON matters(deleted_at) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_chunks_tenant_matter ON document_chunks(tenant_id, matter_id);

-- HNSW Index dla vector search (CRITICAL)
...
-- Helper function: Get user tenant_id
...
BEGIN
  ...
  combined AS (
    SELECT
      ...
      COALESCE(vs.page_number, fts.page_number) AS page_number,
      COALESCE(vs.document_id, fts.document_id) AS document_id,
...
-- ============================================================================
-- SEED DATA (Optional - dla development)
-- ============================================================================

-- Create demo tenant
-- INSERT INTO tenants (name, slug, subscription_status, subscription_plan)
-- VALUES ('Demo Kancelaria', 'demo', 'trial', 'professional');
